...
#import "STRulerView.h"
#import "STRulerCell.h"
#import "UIColor+HexColor.h"
#import "Masonry.h"

@interface STRulerView ()<UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate, UICollectionViewDelegateFlowLayout>
...
-(void)setConfigurations{
    ...
    _smallLineColor = [UIColor blackColor];
    _smallLineHeight = 7;
    _scalesCountBetweenLargeLine = 5;
    _scalesCountBetweenScaleText = 5;
    _alignment = STRulerCenter;
    
    /**   刻度显示文本  */
    _scaleTextColor= [UIColor blackColor];
    ...
    _indicatorHeight = 30;
    _indicatorWidth = 1;
    _indicatorViewColor = [UIColor colorWithHexString:@"#FF0505" alpha:1];
    ...
    /**    自定义刻度相关     */
    _scalesCount = kMaxScalesCount;
    _defaultScale = 0;
    
    /**    归零点原球     */
    _inPointWH = 3;
    _inPointColor = [UIColor blackColor];
...
-(void)setupDataSource{
    ...
    if (self.isShowInPoint && self.isCustomScalesValue && ((self.inPointCurrentScale >self.scalesCount)||(self.inPointCurrentScale<0))) {
    ...
    if (self.isCustomScalesValue && ((self.defaultScale <0)||(self.defaultScale>self.scalesCount))) {
    ...
    if (self.isCustomScalesValue && self.scalesCount == kMaxScalesCount) {
        NSAssert(NO, @"参数错误 刻度尺数值非递增 请设置设置 scalesCount 来确定刻度数");
    }
    ...
    for (int i = 0; i<self.scalesCountBetweenLargeLine; i++) {
        STRulerModel *model = [[STRulerModel alloc]init];
    ...
    //计算所有刻度数
    if (self.isCustomScalesValue) {
        self.totalScale = self.scalesCount;
    }
    else{
    ...
    [self.currentValueLabel setTextColor: [UIColor blackColor]];
    ...
    if (self.isCustomScalesValue) {
        if (self.rulerScaleDidScrollBlock) {
            self.rulerScaleDidScrollBlock(self.defaultScale);
        }
        if (self.rulerScaleDidEndScrollingBlock) {
            self.rulerScaleDidEndScrollingBlock(self.defaultScale);
        }
    }
    else{
        if (self.rulerDidScrollBlock) {
            self.currentValueLabel.text = self.rulerDidScrollBlock(self.defaultValue,CGPointMake(defaultOffset, 0));
        }
        
        if (self.rulerDidEndScrollingBlock) {
            self.currentValueLabel.text = self.rulerDidEndScrollingBlock(self.defaultValue);
        }
    }
}
...
-(void)layoutSubviews{
    ...
    if (self.alignment == STRulerCenter) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    STRulerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"STRulerCell" forIndexPath:indexPath];
    STRulerModel *model = self.dataArray[indexPath.item % self.scalesCountBetweenLargeLine];
    cell.model = model;
    
    NSString *text;
    if (self.isCustomScalesValue) {
        if (indexPath.item % self.scalesCountBetweenScaleText == 0) {
            if (self.rulerScaleTextFormatBlock) {
                text = self.rulerScaleTextFormatBlock(indexPath.item);
            }
            else{
                text = nil;
            }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (self.isCustomScalesValue) {
        maxOffset = self.scalesCount * (self.lineWidth + self.lineSpace)-self.collectionView.contentInset.left;
    }else{
    ...
    if (self.isCustomScalesValue) {
        if (self.rulerScaleDidScrollBlock) {
            self.currentValueLabel.text = self.rulerScaleDidScrollBlock(count);
        }
    } else{
        if (self.rulerDidScrollBlock) {
            self.currentValueLabel.text = self.rulerDidScrollBlock(self.minValue + count*self.unitValue,scrollView.contentOffset);
        }
    }
}
...
-(void)updateRulerLocation{
    ...
    if (self.isCustomScalesValue) {
        ...
        if (self.rulerDidEndScrollingBlock) {
            self.currentValueLabel.text = self.rulerDidEndScrollingBlock(self.minValue + count*self.unitValue);
        }
    }
...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[STRulerCell class] forCellWithReuseIdentifier:@"STRulerCell"];
        //collectionView位置
...
-(UILabel *)currentValueLabel{
    if (!_currentValueLabel) {
        ...
        [_currentValueLabel setTextColor:[UIColor blackColor]];
...
- (CAGradientLayer *)leftGradientLayer{
    if (!_leftGradientLayer) {
        ...
        _leftGradientLayer.colors = @[(__bridge id)[UIColor colorWithHexString:@"#FFFFFF" alpha:0.9].CGColor,(__bridge id)[UIColor colorWithHexString:@"#FFFFFF" alpha:0].CGColor];
...
-(CAGradientLayer *)rightGradientLayer{
    if (!_rightGradientLayer) {
        ...
        _rightGradientLayer.colors = @[(__bridge id)[UIColor colorWithHexString:@"#FFFFFF" alpha:0.9].CGColor,(__bridge id)[UIColor colorWithHexString:@"#FFFFFF" alpha:0].CGColor];
